...
const ErrorBox = ({
  ...
  <div
    ...
    aria-live="assertive"
    className={cn(
      'rounded-xl border border-border-destructive bg-surface-secondary px-3 py-2 text-sm text-text-destructive',
      className,
    )}
  >
    {children}
    ...
    <Suspense fallback={<LoadingFallback />}>
      <DelayedRender delay={DELAYED_ERROR_TIMEOUT}>
        <Container message={message}>
          <div className="mt-2 rounded-xl border border-border-destructive bg-surface-secondary px-4 py-3 text-sm text-text-destructive shadow-sm transition-all">
...
/**
 * Renders directly in `ErrorBox` rather than through `ErrorMessage`/`Error`:
 * this text is already a complete, human-authored sentence, not a raw error
 * code that needs the generic "something went wrong, here's the detail"
 * wrapping `Error.tsx` adds for unrecognized error payloads.
 */
export const UnfinishedMessage = ({ message }: { message: TMessage }) => {
  const localize = useLocalize();
  return (
    <Container message={message}>
      <ErrorBox>{localize('com_aflat_error_incomplete_response')}</ErrorBox>
    </Container>
  );
};
